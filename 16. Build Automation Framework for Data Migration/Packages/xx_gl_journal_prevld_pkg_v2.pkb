  ...
  BEGIN
	...
	LOOP
	  ...
	  EXCEPTION
	    WHEN NO_DATA_FOUND
		THEN
		  lc_err_msg := 'Ledger Not Found -'||SUBSTR(SQLERRM,1,200);
          ln_count := ln_count+1;
	      lt_errors_tbl_typ.EXTEND;
		  lt_errors_tbl_typ(ln_count).entity_name    := lc_entity_name;
	      lt_errors_tbl_typ(ln_count).error_column1  := 'RECORD_ID';
	  ...
	  EXCEPTION
	    WHEN NO_DATA_FOUND
		THEN
		  lc_err_msg := 'Period Not Found -'||SUBSTR(SQLERRM,1,200);
          ln_count := ln_count+1;
	      lt_errors_tbl_typ.EXTEND;
		  lt_errors_tbl_typ(ln_count).entity_name    := lc_entity_name;
	      lt_errors_tbl_typ(ln_count).error_column1  := 'RECORD_ID';
	    ...
	    WHEN OTHERS
		THEN
		  lc_err_msg := 'OTHERS:Period Not Found -'||SUBSTR(SQLERRM,1,200);
          ln_count := ln_count+1;
	      lt_errors_tbl_typ.EXTEND;
		  lt_errors_tbl_typ(ln_count).entity_name    := lc_entity_name;
	      lt_errors_tbl_typ(ln_count).error_column1  := 'RECORD_ID';
	...
	LOOP
	  INSERT INTO xx_data_pre_vld_errors(error_id,object_instance,entity_name,error_column1,error_value1,error_column4,error_value4,error_column5,error_value5,error_message,last_update_date)
	       VALUES (xx_error_id_seq.nextval,p_object_instance,lt_errors_tbl_typ(i).entity_name,lt_errors_tbl_typ(i).error_column1,lt_errors_tbl_typ(i).error_value1,lt_errors_tbl_typ(i).error_column4,lt_errors_tbl_typ(i).error_value4,lt_errors_tbl_typ(i).error_column5,lt_errors_tbl_typ(i).error_value5,lt_errors_tbl_typ(i).error_message, SYSDATE);
	END LOOP;
	COMMIT;
